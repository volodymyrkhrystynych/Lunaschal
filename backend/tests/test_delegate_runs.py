...
def test_run_checkpoints_content_and_flips_to_done(client, monkeypatch):
    ...
    def fake_stream_reply(messages, system_prompt, toolset='chat', **_kwargs):
        yield ('content', 'Hello ')
        yield ('content', 'there.')
        yield ('done', {
...
def test_run_releases_its_lane_slot_even_on_failure(client, monkeypatch):
    ...
    def boom(messages, system_prompt, toolset='chat'):
...
def test_start_runs_on_a_real_thread_and_finishes_the_row(client, monkeypatch):
    ...
    q = runs.start(msg_id, [{'role': 'user', 'content': 'hi'}], '', toolset='chat')
