...
cert so iOS Safari can access the mic), so the listener's defaults —
STT_URL (used by /api/transcribe + /api/tts) and LUNASCHAL_URL (everything
else), both http://127.0.0.1:5000 — would get a connection reset unless
they're pointed at the HTTPS hostname instead. This locks in that handoff.
"""
...
def test_sets_https_urls_in_network_mode(monkeypatch):
    popen_calls = []
    ...
    assert passed_env['STT_URL'] == 'https://kozak.tail20a78b.ts.net:5000'
    assert passed_env['LUNASCHAL_URL'] == 'https://kozak.tail20a78b.ts.net:5000'


def test_leaves_explicit_urls_untouched(monkeypatch):
    popen_calls = []
    env = {
        'STT_LISTENER': '1',
        'TAILSCALE_HOSTNAME': 'kozak.tail20a78b.ts.net',
        'STT_URL': 'http://100.64.0.1:5000',
        'LUNASCHAL_URL': 'http://100.64.0.1:5000',
    }
    _run_start_listener(monkeypatch, env, popen_calls)

    _, passed_env = popen_calls[0]
    assert passed_env['STT_URL'] == 'http://100.64.0.1:5000'
    assert passed_env['LUNASCHAL_URL'] == 'http://100.64.0.1:5000'


def test_no_tailscale_hostname_leaves_defaults_unset(monkeypatch):
    popen_calls = []
    env = {'STT_LISTENER': '1'}
    _run_start_listener(monkeypatch, env, popen_calls)

    _, passed_env = popen_calls[0]
    assert 'STT_URL' not in passed_env
