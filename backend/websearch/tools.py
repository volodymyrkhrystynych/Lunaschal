...
This is the only part of the web-search chat tab that makes arbitrary
outbound requests, and the URLs are chosen by an LLM rather than by the user —
so the SSRF guard below is not optional. The app binds 0.0.0.0 in network
mode and llama-server sits on localhost:8080, which is exactly the kind of
neighbour an unguarded fetcher hands to a prompt injection.
...
the tools return an explanatory *result* rather than raising, so the model
degrades to "answer from what it already knows, and say search was
unavailable" instead of dying mid-loop.
"""
import ipaddress
import re
import socket
...
def _strip_html_with_title(html: str, max_chars: int = MAX_PAGE_CHARS) -> tuple[str, str | None]:
    """(text, title) — the title is what a source link is labelled with."""
...
def is_search_configured() -> bool:
    settings = _settings()
    provider = (settings.get('websearch_search_provider') or '').strip()
    if provider in ('brave', 'tavily'):
        return bool(settings.get('websearch_search_key'))
    if provider == 'searxng':
        return bool(settings.get('websearch_searxng_url'))
    return False
...
def web_search(query: str, limit: int = DEFAULT_RESULTS) -> list[dict]:
    ...
    settings = _settings()
    provider = (settings.get('websearch_search_provider') or '').strip()
    key = settings.get('websearch_search_key') or ''
    ...
    try:
        ...
        if provider == 'searxng':
            url = (settings.get('websearch_searxng_url') or '').strip()
            if not url:
...
def web_fetch(url: str) -> dict:
    ...
    for _ in range(MAX_REDIRECTS + 1):
        ...
        text, title = _strip_html_with_title(body, MAX_PAGE_CHARS)
...
def run_tool(name: str, args: dict) -> tuple[str, dict]:
    ...
    if name == 'web_search':
        ...
        try:
            ...
            return (
                f'Web search is unavailable ({e}). Answer from what you already '
                'know, and say that you could not search the web.',
