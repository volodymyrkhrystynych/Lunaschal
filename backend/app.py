...
def create_app():
    ...
    from backend.routes import journal, calendar, learning, settings, chat, files, writing, stt, tasks, curated_tags, shortcuts, transcriptions, cookbook, food, fanfic, newspapers, meetings, notebook, paper, lifestyle, ideas, practice, memory, email, notes, weather, jobs, backup, logs, repos, life_wiki, piano, torrent
    ...
    if not os.environ.get('LUNASCHAL_NO_SCHEDULERS'):
        ...
        # Needs no llama slot either — it only reconciles our torrent rows
        # against the client and runs a daily retention sweep (08:00–09:00,
        # after the jobs purge). Skips its whole tick when the container
