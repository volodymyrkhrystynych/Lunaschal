...
export function NotebookTree({
  ...
  const dirQueries = useQueries({
    queries: dirPaths.map(p => ({
      queryKey: ['notebook', 'files', 'list', p],
      queryFn: () => api.notebook.files.list(p || undefined),
    })),
  });
  ...
  const visibleNodes = searchOpen
    ? allVisibleNodes.filter(n => matchesQuery(n.entry.name, searchQuery))
    : allVisibleNodes;
  ...
  const renameMutation = useMutation({
    mutationFn: ({ from, to }: { from: string; to: string }) =>
      api.notebook.files.rename(from, to),
    onSuccess: () => {
      queryClient.invalidateQueries({
        queryKey: ['notebook', 'files', 'list'],
      });
  ...
  const createFile = useMutation({
    mutationFn: (name: string) => api.notebook.files.write(name, ''),
    onSuccess: (_, name) => {
      queryClient.invalidateQueries({
        queryKey: ['notebook', 'files', 'list'],
      });
  ...
  const createFolder = useMutation({
    mutationFn: (name: string) => api.notebook.files.mkdir(name),
    onSuccess: () => {
      queryClient.invalidateQueries({
        queryKey: ['notebook', 'files', 'list'],
      });
