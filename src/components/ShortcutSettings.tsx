...
export function ShortcutSettings() {
  ...
  const [draft, setDraft] = useState<Record<ActionId, string>>({
    ...DEFAULT_BINDINGS,
  });
  ...
  const save = useMutation({
    mutationFn: (bindings: Record<string, string>) =>
      api.shortcuts.put(bindings),
    onSuccess: () => {
  ...
  const handleExport = () => {
    const blob = new Blob(
      [JSON.stringify({ version: 1, bindings: draft }, null, 2)],
    ...
    file.text().then(text => {
      try {
        ...
        if (typeof bindings !== 'object' || bindings === null)
          throw new Error('missing "bindings" object');
        const merged = { ...DEFAULT_BINDINGS };
        for (const k of Object.keys(merged) as ActionId[]) {
          const v = bindings[k];
          if (typeof v === 'string' && v) merged[k] = v;
        ...
        setMessage({
          type: 'success',
          text: 'Imported — review and Save to apply',
        });
      } catch (err) {
        setMessage({
          type: 'error',
          text: `Invalid shortcuts file: ${err instanceof Error ? err.message : err}`,
        });
      }
    });
  };
  ...
  return (
    <div className="max-w-2xl">
      ...
      <p className="text-sm text-[var(--color-text-muted)] mb-6">
        Navigate the app with the keyboard: move between tabs, drill into a tab,
        and step through its items. Shortcuts are disabled while you're typing
        in a text field — press{' '}
        <span className="text-[var(--color-text)]">Escape</span> to leave the
        field and get them back. Stored at{' '}
        <code className="text-[var(--color-text)]">./data/shortcuts.json</code>{' '}
