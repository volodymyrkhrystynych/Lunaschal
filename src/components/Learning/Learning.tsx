...
export function Learning() {
  ...
  const { data: tags } = useQuery({
    queryKey: ['learning', 'tags'],
    queryFn: api.learning.getTags,
  });
  const { data: folders } = useQuery({
    queryKey: ['learning', 'folders'],
    queryFn: api.learning.listFolders,
  });
  ...
  return (
    <div className="flex-1 flex flex-col p-4 overflow-hidden">
      <div className="flex items-center justify-between mb-4">
        ...
        <div className="flex gap-2">
          <button
            ...
            className={modeClass(mode === 'review', level === 1)}
          >
            Review ({stats?.due ?? 0})
          </button>
          <button
            onClick={() => setMode('queue')}
            className={modeClass(mode === 'queue', level === 1)}
          >
            Queue
            {pendingCount > 0 && (
              <span className="ml-1.5 px-1.5 py-0.5 text-xs rounded-full bg-orange-500/30 text-orange-300">
                {pendingCount}
              </span>
            )}
          </button>
          <button
            ...
            className={modeClass(mode === 'browse', level === 1)}
          >
            Browse
          </button>
          <button
            onClick={() => setMode('create')}
            className={modeClass(mode === 'create', level === 1)}
          >
            + Create
          </button>
          <button
            onClick={() => setMode('folders')}
            className={modeClass(mode === 'folders', level === 1)}
          >
            Folders
          </button>
        </div>
      ...
      <div ref={scrollRef} className="flex-1 overflow-y-auto">
        ...
        {mode === 'create' && (
          <BrainDump folderId={folderId} onGenerated={() => setMode('queue')} />
        )}
