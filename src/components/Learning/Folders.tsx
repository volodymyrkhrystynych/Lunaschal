    ...
    <div className="max-w-2xl mx-auto space-y-6">
      <div className="bg-[var(--color-surface)] rounded-lg border border-white/10 p-5">
        ...
        <p className="text-sm text-[var(--color-text-muted)] mb-4">
          A folder scopes which evidence provider verification uses. Cards
          without a folder can't be verified.
        </p>
        ...
        <div className="flex gap-2">
          <input
            ...
            onKeyDown={e => {
              if (e.key === 'Enter' && newFolder.trim()) createFolder.mutate();
            }}
...
function McpServers({ servers }: { servers: McpServer[] }) {
  ...
  const invalidate = () =>
    queryClient.invalidateQueries({ queryKey: ['learning', 'mcp-servers'] });

  const create = useMutation({
    mutationFn: () =>
      api.learning.createMcpServer({
        name: form.name.trim(),
  ...
  return (
    <div className="bg-[var(--color-surface)] rounded-lg border border-white/10 p-5">
      <h2 className="text-lg font-semibold text-[var(--color-text)] mb-1">
        Evidence providers (MCP servers)
      </h2>
      <p className="text-sm text-[var(--color-text-muted)] mb-4">
        e.g. Context7 for current library docs: command{' '}
        <code className="text-xs bg-white/5 px-1 rounded">npx</code>, args{' '}
