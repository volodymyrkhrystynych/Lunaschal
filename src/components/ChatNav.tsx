  ...
  return (
    <div className="w-64 shrink-0 border-r border-white/10 bg-[var(--color-surface)] flex flex-col">
      <div className="flex items-center justify-between px-2 pt-3 pb-1">
        <span className="text-xs font-semibold text-[var(--color-text-muted)] uppercase tracking-wider">Conversations</span>
        <button
      ...
      <div className="flex-1 overflow-y-auto p-2 pt-1">
        {conversations?.map((conv) => (
          <div key={conv.id} className={rowClass(conv.id)} onClick={() => onSelect(conv.id)}>
            <span className="text-sm truncate flex-1 min-w-0">{conv.title || 'New Conversation'}</span>
            <button
              onClick={(e) => {
                e.stopPropagation();
                if (confirm(`Delete "${conv.title || 'New Conversation'}"?`)) deleteConversation.mutate(conv.id);
              }}
        ...
        {conversations && conversations.length === 0 && (
          <div className="text-sm text-[var(--color-text-muted)] px-2 py-1">No conversations yet</div>
        )}
      </div>
    </div>
  );
