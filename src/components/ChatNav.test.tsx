...
describe('ChatNav', () => {
  it('renders conversation titles and highlights the selected one', async () => {
    renderWithProviders(
      <ChatNav currentConversationId="c2" onSelect={() => {}} />
    );
    ...
    expect(
      screen.getByText('Morning plans').parentElement!.className
    ).not.toContain('bg-[var(--color-primary)]/20');
  });

  it('selects a conversation on click and starts a new one via +', async () => {
    const onSelect = vi.fn();
    renderWithProviders(
      <ChatNav currentConversationId={null} onSelect={onSelect} />
    );
  ...
  it('deletes a conversation and clears selection when it was current', async () => {
    const onSelect = vi.fn();
    renderWithProviders(
      <ChatNav currentConversationId="c1" onSelect={onSelect} />
    );
    ...
    await vi.waitFor(() =>
      expect(vi.mocked(api.chat.deleteConversation).mock.calls[0][0]).toBe('c1')
    );
  ...
  it('shows an empty state when there are no conversations', async () => {
    ...
    renderWithProviders(
      <ChatNav currentConversationId={null} onSelect={() => {}} />
    );
