...
export function Setup() {
  ...
  const setup = useMutation({
    mutationFn: api.settings.setup,
    onSuccess: () => queryClient.invalidateQueries({ queryKey: ['settings', 'isSetupComplete'] }),
    ...
    <div className="min-h-screen bg-[var(--color-bg)] flex items-center justify-center p-4">
      <div className="w-full max-w-md">
        <div className="text-center mb-8">
          ...
          <p className="text-[var(--color-text-muted)]">Your personal AI knowledge assistant</p>
        </div>
        ...
        <form onSubmit={handleSubmit} className="bg-[var(--color-surface)] rounded-lg border border-white/10 p-6">
          ...
          <div className="space-y-4">
            ...
            <div>
              <label className="block text-sm text-[var(--color-text-muted)] mb-1">Confirm Password</label>
              <input type="password" value={confirmPassword} onChange={(e) => setConfirmPassword(e.target.value)}
                placeholder="Enter password again"
                className="w-full bg-transparent text-[var(--color-text)] border border-white/10 rounded-lg px-4 py-2 focus:outline-none focus:border-[var(--color-primary)]" />
            </div>
            <button type="submit" disabled={!password || !confirmPassword || setup.isPending}
