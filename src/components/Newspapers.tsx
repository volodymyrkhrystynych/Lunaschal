...
export function Newspapers() {
  ...
  const sync = useMutation({
    ...
    onSuccess: () =>
      queryClient.invalidateQueries({ queryKey: ['newspapers', today] }),
  });
  ...
  return (
    <div className="flex-1 flex flex-col p-4 overflow-hidden">
      ...
      <div className="flex items-center justify-center gap-3 mb-4">
        ...
        <h2 className="text-lg font-medium text-[var(--color-text)] w-72 text-center">
          {formatDisplayDate(selectedDate)}
        </h2>
        <button
          onClick={() => setSelectedDate(d => shiftDateISO(d, 1))}
          disabled={isFutureDate(shiftDateISO(selectedDate, 1))}
          className="p-2 text-[var(--color-text-muted)] hover:text-[var(--color-text)] disabled:opacity-30 disabled:cursor-not-allowed"
        >
          →
        </button>
        <input
