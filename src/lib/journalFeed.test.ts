...
function entry(id: string, createdAt: string): JournalEntry {
  return {
    id, createdAt,
    content: `entry ${id}`, rawContent: null, title: null, tags: null,
    curatedTags: [], updatedAt: createdAt,
  };
}

function transcription(id: string, createdAt: string): Transcription {
  return { id, createdAt, text: `transcription ${id}`, source: 'paste', app: null, detail: null };
}
...
describe('buildFeed', () => {
  ...
  it('passes through entries only, preserving order and index', () => {
    const entries = [entry('a', '2026-07-08T12:00:00'), entry('b', '2026-07-07T12:00:00')];
  ...
  it('passes through transcriptions only', () => {
    const ts = [transcription('t1', '2026-07-08T12:00:00'), transcription('t2', '2026-07-07T12:00:00')];
  ...
  it('interleaves by createdAt descending', () => {
    const entries = [entry('e1', '2026-07-08T12:00:00'), entry('e2', '2026-07-06T12:00:00')];
    const ts = [transcription('t1', '2026-07-07T12:00:00'), transcription('t2', '2026-07-05T12:00:00')];
    const feed = buildFeed(entries, ts);
    expect(feed.map((i) => (i.kind === 'entry' ? i.entry.id : i.transcription.id)))
      .toEqual(['e1', 't1', 'e2', 't2']);
  });
