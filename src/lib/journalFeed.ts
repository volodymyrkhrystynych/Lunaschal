...
// chat day, or an archived paper (drawings). `entryIndex` preserves the entry's
// position in the original entries array so keyboard navigation (selIndex) keeps
// indexing entries only — the other kinds are not selectable.
...
// Merge entries, transcriptions, saved chats, and archived papers into one feed
// sorted by time descending (conversations sort by updatedAt, papers by the
// moment they were archived). All inputs are already newest-first; entries win
// exact ties.
export function buildFeed(
  ...
  while (
    ...
    if (
      ...
      e++;
    } else if (tr && trTime >= convTime && trTime >= paperTime) {
      feed.push({ kind: 'transcription', transcription: tr });
      t++;
    } else if (conv && convTime >= paperTime) {
