...
def seed_journal(db):
    ...
    #
    # The EXIF block is written by hand for the same reason: the placeholder is
    # generated by Pillow and carries no tags at all, so a seeded photo would
    # come back with null metadata and nothing to look at. This is the shape a
    # real phone capture produces (backend/food/exif.py's extract_exif_block),
    # minus the GPS map the camera path does not supply.
    db.execute(
        'INSERT INTO journal_attachments (id, entry_id, kind, name, path, mime, size, position,'
        ' latitude, longitude, exif, created_at) '
        'VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)',
        (attachment_id, entry_id, 'image', 'fixed drawer', str(img_path), 'image/jpeg',
         img_path.stat().st_size, 0, entries[-1][5][0], entries[-1][5][1],
         json.dumps({
             'ImageWidth': 1280, 'ImageHeight': 960,
             'Make': 'Apple', 'Model': 'iPhone 15 Pro',
             'LensModel': 'iPhone 15 Pro back triple camera 6.765mm f/1.78',
             'Software': '17.4.1', 'Orientation': 1,
             'DateTimeOriginal': '2026:03:14 09:41:02',
             'ExposureTime': 0.0166, 'FNumber': 1.78,
             'ISOSpeedRatings': 125, 'FocalLength': 6.765, 'Flash': 16,
         }),
         ts(1)),
    )
