...
- **The delegate proposes; it never writes.** `propose_task` / `propose_calendar_event` / `propose_calorie_log` / `propose_note_to_self` / `propose_flashcards` stage a payload and return "nothing has been saved yet" to the model, which is what stops the reply claiming it's done. The user's click on the existing confirm card is still the only thing that inserts a row, through the unchanged `/api/chat/save-*` routes. `web_search` / `web_fetch` do run for real.
...
- **One `priority.begin('chat.stream')` spans the whole turn**, sub-loop included, acquired in the view and released in the generator's `finally` — same shape as `ideas.discuss`.
- SSE carries four kinds under the one `data:` frame: a bare `{tool: …}` step, `{thinking: …}`, `{content: …}`, and a final `{done: true, steps, sources, proposals}`. Steps ride the `done` event _as well as_ streaming live, because the browser persists them onto the message metadata and a reload has to redraw the same trace.
