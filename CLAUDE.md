...
#### Torrents — see [docs/torrent-tab.md](docs/torrent-tab.md) for why the traffic cannot leak (qBittorrent shares gluetun's network namespace, so the kill switch is structural rather than a setting), why a container rather than a root `ip netns`, the three container details that are easy to get wrong, ProtonVPN port-forward syncing, and what exFAT costs.
