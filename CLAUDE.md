...
- **A view can be gated to a device, and the two gates mean different things.** `desktopOnly` in `navItems` is the native PyWebView shell (Piano); `largeOnly` is a viewport wide enough for the view to work at all (Study, `min-width: 1024px`). The Pocket 2 satisfies the first and fails the second, which is the whole reason there are two. Both go through the one pure `visibleNavItems` in `src/lib/navVisibility.ts`, which `Sidebar` and `App` (for `availableViews`) share; `App` also falls back to `'chat'` when the persisted view is one this device can't show
