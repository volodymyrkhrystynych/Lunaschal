...
- (`STT_SCREENSHOT_KEY`) — capture the full desktop → save a photo on a new journal entry. Rebind or disable **Screenshot to journal** in Settings beside the voice shortcuts, then restart the listener. Requires `grim` and a Wayland compositor supporting its screen capture protocol; capture happens on the listener's machine even when the server is remote. No key is assigned by default. An explicitly disabled setting overrides the env fallback.

Screenshots are captured by `stt/screenshots.py` to private files under `$XDG_DATA_HOME/lunaschal/screenshots` (default `~/.local/share/lunaschal/screenshots`). Completed PNGs remain there until both journal creation and attachment upload succeed. The listener retries on startup and every minute using the same ULID for replay, and reports capture/upload outcomes through desktop notifications when `notify-send` is available, plus its log. The full desktop includes all connected outputs. Capturing never requires or waits for transcription or AI.
