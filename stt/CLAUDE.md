...
- (`STT_SCREENSHOT_KEY`) — capture only the focused monitor → append a photo to the current screenshot journal entry. Consecutive captures share that entry until it is edited or another journal entry is created. Rebind or disable **Screenshot to journal** in Settings beside the voice shortcuts, then restart the listener. Requires Hyprland (`hyprctl`) and `grim`; capture happens on the listener's machine even when the server is remote. No key is assigned by default. An explicitly disabled setting overrides the env fallback.

Screenshots are captured by `stt/screenshots.py` to private files under `$XDG_DATA_HOME/lunaschal/screenshots` (default `~/.local/share/lunaschal/screenshots`). Completed PNGs remain there until the one-shot screenshot upload confirms the attachment and grouped journal entry. The listener retries on startup and every minute using the same ULID for replay. The backend stores the listener's offset-bearing capture time and names each attachment `YYYY-MM-DD HH:MM:SS` in that local time. The first capture has no calendar event because it has no span; the second creates one from the first capture minute through the second, and later captures extend it. Deleting an individual screenshot leaves the event alone. It reports capture/upload outcomes through desktop notifications when `notify-send` is available, plus its log. Every capture resolves the focused output with `hyprctl -j monitors` and passes its name to `grim -o`; an unknown focus fails rather than capturing all monitors. Capturing never requires or waits for transcription or AI.
